...
if __name__ == '__main__':
    ...
    for i, frame in enumerate(frames):
        ...
        # [修复] 只取文件名(如 0000.png)，忽略 JSON 中的复杂相对路径
        file_name = os.path.basename(file_path)
        
        # 读取 RGB
        rgb_full_path = os.path.join(args.rgb_path, file_name)
        rgb = cv2.imread(rgb_full_path)
        if rgb is None:
            print(f"[!] Error: Cannot read RGB file: {rgb_full_path}")
            continue
        ...
        # 读取 Depth
        depth_full_path = os.path.join(args.depth_path, file_name.replace("png","exr"))
        depth_img = cv2.imread(depth_full_path, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
        if depth_img is None:
            print(f"[!] Error: Cannot read Depth file: {depth_full_path}")
            continue

        depth = depth_img[...,0] / 10000. # cm -> 100m
